...
describe('selectorArgsToStateKey', () => {
	...
	it('should remove trailing undefined values', () => {
		expect(selectorArgsToStateKey([1, 2, undefined])).toEqual([1, 2]);
		expect(selectorArgsToStateKey([1, 2, undefined, undefined])).toEqual([
			1, 2,
		]);
	});

	it('should leave non-trailing undefined values alone', () => {
		expect(selectorArgsToStateKey([1, undefined, 2, undefined])).toEqual([
			1,
			undefined,
			2,
		]);
