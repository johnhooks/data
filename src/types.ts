...
import type { combineReducers as reduxCombineReducers, Action } from 'redux';
...
export interface StoreInstance<Config extends AnyConfig> {
	/**
	 * Get the store's selectors.
	 */
	getSelectors: () => SelectorsOf<Config>;

	/**
	 * Get the store's action creators.
	 */
	 ...
	 *
	 * @param listener The callback to run when the store data has changed.
	 */
	subscribe: (listener: () => void) => () => void;

	/**
	 * The store's resolvers.
	 *
	 * @private
	 */
	resolvers?: ResolversOf<Config>;
}
...
export type CurriedSelectorsOf<S> =
...
/**
 * The selectors for a given store.
 */
...
/**
 * The selector name type for a given store.
 *
 * @todo: Is this useful?
 */
export type SelectorNamesOf<Config extends AnyConfig> =
	Config extends ReduxStoreConfig<any, any, infer Selectors>
		? keyof Selectors
		: string;

/**
 * The selector resolvers for a given store.
 *
 * The same possible keys are selectors.
 */
export type ResolversOf<Config extends AnyConfig> =
	Config extends ReduxStoreConfig<any, any, infer Selectors>
		? {
				[name in keyof Selectors]: Function & {
					shouldInvalidate: (
